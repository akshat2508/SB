...
function validation(){
  ...
  function find_sequence(){
    ...
    document.getElementById("dangerSign").style.display = "none"; // Hide danger sign initially
    ...
    for(var j=1; j<=5; j++){
        ...
        for(var i=k; i<=5; i++){
            ...
            if(ex1!= 0 || ex2!= 0 || ex3!= 0){
                if(x1 >= parseInt(document.getElementById('n'+i+'1').value) &&
    ...
    if (checker == 0) {
        document.getElementById("calc0").innerHTML = "<span style='color: red; font-size: 20px; font-weight: bold;'>⚠️ Deadlock Detected!</span>";
    }
